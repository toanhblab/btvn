...
import { extractAssignments, hasAI, splitByRule } from '@/lib/ai';
...
export async function POST(req: Request) {
  ...
  if (hasAI) {
    try {
      ...
      if (drafts.length > 0) return NextResponse.json({ drafts, source: 'ai' });
      ...
      return NextResponse.json({
        drafts: text.trim() ? splitByRule(text) : [],
        source: 'rule',
