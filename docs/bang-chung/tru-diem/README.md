...
| `05-bome-go-qua-so-moi-tru-het-5-dien-thoai.png` | Tầng giao diện của "không âm": chọn An (5 ⭐), gõ 8 → chữ đỏ "Con chỉ có 5 ⭐" **và** cái nút đổi luôn thành **"Trừ hết 5 ⭐"** (bấm được) — không phải nút khoá bắt bố mẹ gõ lại. Cùng một nút, cùng một hàm `trangThaiTruDiem`, với đường máy chủ từ chối ở ảnh 06. |
| `06-bome-may-chu-tu-choi-moi-tru-het-3-dien-thoai.png` | Tầng dữ liệu của "không âm": gõ 4 (≤ 5 đang hiện) và chọn "Cãi bố mẹ"; trong lúc đó máy khác trừ An 2 ⭐ (gọi API từ console) → bấm "Trừ 4 ⭐ của An", máy chủ từ chối "An chỉ còn 3 ⭐, không trừ được 4 ⭐.", viên An và dòng "con đang có" cập nhật 3, nên nút hiện **"Trừ hết 3 ⭐"** (kèm chữ "Con chỉ có 3 ⭐") — N là số máy chủ vừa tính, không phải 5 đang hiện lúc mở form; bấm là gửi `truHet: true`, máy chủ tính lại N một lần nữa. |
