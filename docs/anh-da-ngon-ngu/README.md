...
- `flow-doi-ngon-ngu-bang-pin.gif` — luồng đổi ngôn ngữ **chỉ bằng nhập PIN**:
  màn PIN (tiếng Việt) → 1111 → tổng quan + màn chọn con tiếng Nhật → quên PIN →
  màn PIN (giờ đã tiếng Nhật, vì máy gắn với nhà Nhật) → 2222 → tiếng Hàn → 3333
  → tiếng Anh. Mỗi khung một giây (`ffmpeg -framerate 1`, có palettegen/paletteuse).
