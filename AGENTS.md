...
định): khoá `pg_advisory_xact_lock` theo con → `INSERT … SELECT` ghi `LEAST(số gõ,
số dư tính tại chỗ)` và chỉ ghi khi số dư > 0 → đọc số dư. Đừng kẹp
`GREATEST(0, …)` trong SUM. **"Không âm" là luật của SỐ DƯ, không của riêng một
đường**: `duyetDoiThuong` cũng là một đường trừ
...
lặp lại đúng khuôn này, đừng đọc số dư ở một câu rồi ghi ở câu sau. **Chỉ có MỘT
đường gửi**: màn luôn gửi đúng số bố mẹ gõ, máy chủ ghi `LEAST(số gõ, số dư)` nên
số trên màn cũ không bao giờ trừ quá số đã gõ (gõ 8 lúc con vừa kiếm thêm là trừ 8,
không phải trừ sạch); gõ quá số dư thì nút chỉ ĐỔI MẶT thành "Trừ hết N" (một hàm
thuần `trangThaiTruDiem`), và lý do từ chối duy nhất là con không còn ⭐ nào. Lý do
là thứ CON ĐỌC ở cửa hàng (`LY_DO_TRU_GOI_Y`, `LY_DO_TRU_TRONG` trong `lib/types.ts`).
